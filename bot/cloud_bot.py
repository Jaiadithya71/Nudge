...
# Add project path for message queue import
...
class UserIntent(BaseModel):
    ...
    command: str = Field("", description="The whitelisted command to execute on the laptop. Must be empty or one of: 'git status', 'git log -n 5', 'run tests', 'screenshot', or start with 'query DB ' followed by a name to search in the database (e.g. 'query DB hareshreenath'). Choose this if the user asks to query info, search the database, or get photos/records of a student or person from the database.")
...
async def online_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    processing_msg = await update.message.reply_text("Checking health status...")
    try:
        health = message_queue.get_antigravity_health()
        
        status_emoji = "🟢" if health["status"] == "active" else "🔴"
        status_text = "Active" if health["status"] == "active" else "Possibly Down"
        
        last_response_str = "Never"
        if health["last_response"]:
            last_response_str = f"{health['last_response']} (approx. {health['age_minutes']} min ago)"
            
        msg = (
            f"🧠 *Antigravity Health Status*\n\n"
            f"Status: {status_emoji} {status_text}\n"
            f"Last Response: {last_response_str}\n"
            f"Pending Messages: {health['pending_messages']}\n"
            f"Direct Commands: 🟢 Available\n"
        )
        await processing_msg.edit_text(msg, parse_mode="Markdown")
    except Exception as e:
        await processing_msg.edit_text(f"Error checking health: {str(e)}")

async def outbox_monitor_job(context: ContextTypes.DEFAULT_TYPE):
    try:
        ready_msgs = message_queue.read_ready_outbox()
        for msg in ready_msgs:
            ...
            message_queue.mark_outbox_delivered(msg_id)
            
            # Periodically clean up old messages
            message_queue.cleanup_old_messages(max_age_hours=24)
    except Exception as e:
        print(f"Error in outbox monitor job: {e}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith(">"):
        ...
        processing_msg = await update.message.reply_text(f"Executing direct command: {command_text}...")
        
        try:
            sys.path.append(os.path.dirname(os.path.abspath(__file__)))
            from command_handlers import dispatch_command
            
            nudge_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            res = dispatch_command(command_text, nudge_dir, {})
            
            await processing_msg.delete()
            
            if isinstance(res, tuple) and len(res) == 3 and res[0] == "photo":
                _, photo_path, caption = res
                if os.path.exists(photo_path):
                    with open(photo_path, "rb") as photo:
                        await update.message.reply_photo(photo=photo, caption=caption[:1024])
                else:
                    await update.message.reply_text(f"Error: Photo not found at {photo_path}")
            else:
        ...
        # Normal message -> queue for Antigravity
        try:
            message_queue.write_to_inbox(text)
            await update.message.reply_text("🧠 Sent to Antigravity. Processing...")
        except Exception as e:
            await update.message.reply_text(f"Error queueing message: {str(e)}")
...
async def main():
    ...
    application.job_queue.run_repeating(outbox_monitor_job, interval=10, first=5)
